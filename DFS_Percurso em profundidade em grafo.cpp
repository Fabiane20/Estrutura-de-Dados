...
int main(){
  ...
  if( grafo != NULL )      // Verifica se grafo não está vazio
  	  bfs( grafo, 0 );     // Percurso em profundidade no grafo a partir do vértice indicado como argumento
...
void bfs(struct GRAFO* grafo, int vertice){
int inicio = 0, final = 0; // Váriaveis para controlar o índice de inicio e fim da fila

for(int i = 0; i < grafo->numVertices; i++) // Coloca todos os vertices como não visitados
grafo->visitado[i] = 0;

int* fila = (int*)malloc(grafo->numVertices * sizeof(int));  //Cria e armazena o espaço de uma fila com p tamanho de 4 int

fila[final++] = vertice;   // Adicionamos esse vertice ao final da fila
    grafo->visitado[vertice] = 1; // Marca como vértice visitado

    while (inicio < final) { // Para percorrer a fila
        int verticeAtual = fila[inicio++]; // Retira o vértice do início da fila e incrementa o índice de início
        printf( "Visitado %d \n", verticeAtual); // Retorna o vértice visitado

        struct NODO* temp = grafo->adjListas[verticeAtual];     // Pega a lista de adjacência do vértice atual
        while (temp) { // Enquanto tiver nós na lista de adjacência
            int adjVertice = temp->vertice; // Pega o vértice adjacente atual
            if (grafo->visitado[adjVertice] == 0) { // Verifica se ele não foi visitado
                grafo->visitado[adjVertice] = 1; // Marca como visitado
                fila[final++] = adjVertice; // Adiciona o vértice adjacente ao final da fila
            }
            temp = temp->prox; // Vai para o próximo vértice
        }
    }
